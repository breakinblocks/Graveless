...
public class GraveBrowserScreen extends Screen {
    ...
    private void requestDetail(UUID id) {
        if (!details.containsKey(id)) {
            ClientPacketDistributor.sendToServer(new GravelessNetworking.GraveDetailRequestPayload(ownerId, id));
        }
    }
    ...
    private void drawTerrainPanel(GuiGraphicsExtractor graphics, Layout l) {
        ...
        if (blocks != null) {
            ...
            graphics.submitPictureInPictureRenderState(new GraveDioramaRenderState(
    ...
    public boolean mouseClicked(MouseButtonEvent event, boolean doubleClick) {
        ...
        if (admin && event.hasShiftDown() && selectedId != null) {
            ...
            if (index >= 0) {
                click();
                ClientPacketDistributor.sendToServer(new GravelessNetworking.GraveExtractPayload(
        ...
        if (viewingSelf()) {
            ...
            if (hovered(mouseX, mouseY, l.listX(), y, half, BUTTON_H)) {
                click();
                ClientPacketDistributor.sendToServer(new GravelessNetworking.ProfileActionPayload(
        ...
        if (hasSelection && hovered(mouseX, mouseY, deleteX, l.deleteY(), 100, BUTTON_H)) {
            click();
            if (confirmDelete) {
                ClientPacketDistributor.sendToServer(new GravelessNetworking.GraveActionPayload(
            ...
            case OVERLAY_PICKER -> {
                if (inRows && rowIndex >= 0 && rowIndex < players.size()) {
                    ...
                    if (!entry.id().equals(ownerId)) {
                        ClientPacketDistributor.sendToServer(new GravelessNetworking.GraveViewRequestPayload(entry.id()));
                    }
                }
            }
            case OVERLAY_ACCESS -> {
            ...
            case OVERLAY_BACKUPS -> {
                if (inRows && rowIndex >= 0 && rowIndex < backups.size()) {
                    ...
                    if (armedBackup == rowIndex) {
                        ClientPacketDistributor.sendToServer(new GravelessNetworking.BackupRevivePayload(
    ...
    private void runAction(int action) {
        switch (action) {
            ...
            case ACTION_TRACK -> ClientPacketDistributor.sendToServer(new GravelessNetworking.GraveActionPayload(
                    ownerId, selectedId, GravelessNetworking.GraveActionPayload.ACTION_TRACK));
            case ACTION_RESTORE -> ClientPacketDistributor.sendToServer(new GravelessNetworking.GraveActionPayload(
                    ownerId, selectedId, GravelessNetworking.GraveActionPayload.ACTION_RESTORE));
            case ACTION_BACKUPS -> {
                openOverlay(OVERLAY_BACKUPS);
                ClientPacketDistributor.sendToServer(new GravelessNetworking.BackupListRequestPayload(ownerId));
            }
            default -> {
            }
        }
